...
{
    // Static variable to hold the file handle. It is initialized to NULL
    // only once and persists between function calls.
    static FILE* logFile = NULL;

    // This block runs only on the very first call to this function.
    if (logFile == NULL) {
        // Attempt to open "log.txt" in "append" mode.
        ...
        if (logFile == NULL) {
            ...
            // ...and set the log file to stdout to prevent future file-opening attempts.
            logFile = stdout;
        }
    }

    // Get the current time to create a timestamp.
    char timeStr[64];
    ...
    // Flush the stream to ensure the log is written immediately.
    fflush(logFile);
}
...
void *goose_thread_task(void *arg)
{
    ...
    // printf("goose_thread_task ::Thread started for appid 0x%d on interface %s\n", data->AppID, data->interface);
